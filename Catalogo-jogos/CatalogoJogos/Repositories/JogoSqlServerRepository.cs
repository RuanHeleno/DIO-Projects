...
namespace CatalogoJogos.Repositories
{
    public class JogoSqlServerRepository : IJogoRepository
    {
        ...
        public async Task<List<Jogo>> Obter(int pagina, int quantidade)
        {
            ...
            var comando = $"select * from Jogos by id offset {((pagina -1) * quantidade)} rows fetch next {quantidade} rows'";
        ...
        {
            var comando = $"insert into Jogos values ('{jogo.Id}', '{jogo.Nome}', '{jogo.Produtora}', '{jogo.Preco.ToString()}')";
        ...
        public async Task Atualizar(Jogo jogo)
        {
            var comando = $"update Jogos set Nome = '{jogo.Nome}', Produtora = '{jogo.Produtora}', " +
                $"Preco = '{jogo.Preco.ToString()}', from Jogos where Id = '{jogo.Id}'";
